...
- Colors (and their semantic roles), typography scale, component states, spacing/elevation, responsive rules, Do's and Don'ts.

Rules:
- Never introduce tokens/components that conflict with DESIGN.md. If the spec demands something unlisted, extend DESIGN.md (append + note) rather than hardcoding ad-hoc values.
- If DESIGN.md is missing for a UI-heavy project, stop and request ui-designer to author it first — don't improvise a parallel system.
- Cite DESIGN.md sections in commit/iteration notes when visual decisions are non-obvious.
