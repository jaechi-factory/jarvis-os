...
**나머지 55개 PM Skills**: `SKILL.md`의 description 기반 자동 로드에 맡김. 필요 시 {{USER_NAME}}이 `/플러그인명:스킬명` 직접 호출.
...
핵심: context7(라이브러리 문서)·figma(디자인 URL 즉시)·superpowers(브레인스톰·계획·TDD 등) 자동 호출. {{USER_NAME}}이 외울 필요 없이 키워드 매칭으로 발동.
