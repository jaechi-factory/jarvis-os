...
description: 32 Leader 에이전트(핵심 23+번들 9) + 218 Worker 스킬 + 7 Infra MCP 전수 분류. 직군 매핑·빈도·상태 라벨. 미사용 후보 리스트 포함 (ben 픽용). v1.4 (2026-04-25): Profile v2.0 풀스택 올라운더 반영, Leader 빈도 5개 격상
type: reference
...
ben(Founder)이 "내 도구 박스 정리하자"고 요청. 이 카탈로그는:
- L3 Leader 32개·L4 Worker ~218개·Infra 7개를 **한 화면에** 노출
- 직군 매핑(`feedback_role_tool_matrix`)에서 1·2순위로 박힌 도구는 "활성", 매핑 0건은 "미사용 후보"
- ben이 후보 리스트 보고 직접 픽(유지/비활성/무시) → 불필요한 노이즈 제거 SSOT
- **삭제·비활성은 안 함**. 이 문서는 분류·라벨링까지만. 비가역 액션은 ben 명시 승인 필요
...
| 빈도 | 카운트 | 설명 |
|---|---|---|
| 🔥 자주 | ~35 | 매트릭스 1순위 또는 ben 매일 사용 (ux-write, codex, gui-critic, architect, typescript-reviewer 등). 풀스택 5개 격상 |
| 🟡 가끔 | ~75 | 매트릭스 2순위 또는 케이스 한정 사용 (디자인 15 + GAN 5 + 신규 활성화 분 포함) |
| ⚪ 거의 안 씀 | ~153 | 매트릭스 매핑 0건 + 추정 미사용. **ben 픽 후보** (v1.3까지 30개 비활성 보관 분리) |
...
매트릭스 매핑 0건 + 매일 작업과 거리 먼 도구. **삭제·비활성 안 함**. ben이 카테고리별로 픽:
...
| pm-product-discovery:identify-assumptions-new/existing | 추상적 분석, ben 사용 흔적 없음 | □ □ □ |
...
### 6-2. Designer Skills (이전 미사용 9개) → ✅ 활성 재배분 완료 (2026-04-25 ben 지시)

ben이 "이번에 받은 디자인 플러그인은 사용 쪽으로 배분"하라고 명시. 9개 모두 ⚪ → 🟡 활성(보조). 신규 매핑은 `feedback_design_tool_routing.md` 참조.
...
| 스킬 | 사유 | ben 픽 |
|---|---|---|
| ce-compound, ce-compound-refresh | "knowledge compounding" 메타, ben 사용 패턴 외 | □ □ □ |
...
### 6-4. UI-UX-Pro-Max (이전 미사용 6개) → ✅ 활성 재배분 완료 (2026-04-25 ben 지시)
...
| version-bump | 플러그인 개발 시만 (ben 작업 외) | □ □ □ |

### 6-6. Leader 에이전트 (이전 미사용 8개) → ✅ GAN 5개 활성 재배분 (2026-04-25 v1.2, ben 사용 패턴 + Profile v2.0)

ben이 "GAN 한 번씩 잘 썼다" 명시 + User Profile v2.0(풀스택 올라운더) 반영. GAN 시리즈 + 페어 도구 5개는 활성 유지.
...
— v1.2: Leader GAN 5개 활성 재배분 (ben 실사용 패턴 + Profile v2.0)
— v1.3: 명확한 9개(CE 5 + PM "new" 4) **비활성 보관** (ben "전부 허가" 명시 승인)
...
— 남은 ben 픽 대상: PM Skills 14 + Compound-Engineering 10 + Claude-Mem 3 + Leader 3 = **30개**

---

## 6-7. 비활성 보관 (자동 매칭 제외) — v1.3 (2026-04-25)

ben "비활성 진행" 명시 승인. L1 자동 매칭에서 이 9개 제외. 직접 호출 시(`/플러그인:스킬명`)는 여전히 작동. 가역적 — 다시 활성화 원하면 이 섹션에서 빼면 됨.
...
- **빈도 추정 기반**: 실제 호출 로그 분석 X. 매트릭스 1순위 매핑 + ben 일상 작업 추정으로 라벨링.
...
- **삭제·비활성 결정은 ben 권한**: 이 카탈로그는 분류·라벨링까지. 비가역 액션은 명시 승인 필요.
...
- **ben 픽 결과 반영**: 미사용 후보 리스트에서 비활성 결정된 도구는 별도 섹션 8 "비활성 보관" 추가
