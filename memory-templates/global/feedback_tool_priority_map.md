...
{{USER_NAME}}이 한국 PD라 같은 카테고리에 여러 도구가 있으면 매번 후보 비교하느라 일관성 흔들림. 비교 라운드 1회로 케이스별 1순위를 정하고 직진하기 위함.
...
3. **새로 자주 충돌하는 케이스 발견 시** → {{USER_NAME}}에 보고하고 비교 라운드 추가 후 이 맵 업데이트.
