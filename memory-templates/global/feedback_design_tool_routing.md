...
description: 디자인 작업 시 도구 호출 우선순위 룰. 혼합 모델(해석 C) — 탐색·시안·시스템은 새 디자인 스킬 디폴트, 평가·감도·라이팅은 자체 에이전트(한국어 SSOT). 2026-04-25 {{USER_NAME}} 지시로 받은 디자인 도구 15개 활성 재배분
type: feedback
...
**작성**: 2026-04-25
**근거**: {{USER_NAME}} 명시 지시 — "디자인쪽은 이번에 받은 것들 위주로 세팅" + "미사용에서 이번에 받은 것은 사용 쪽으로 배분"
...
2026-04-25에 디자인 마켓플레이스 3개(designer-skills 8 플러그인, frontend-design-audit, ui-ux-pro-max-skill) + Figma 공식 플러그인 새로 설치. {{USER_NAME}}이 "받은 도구 활용도 올려라"고 지시 → 디자인 도구 호출 디폴트를 새 도구 우선으로 보강하면서, 한국어 우위 자체 에이전트 SSOT는 유지.
...
## 활성 재배분 결과 (15개, {{USER_NAME}} 지시)

이전 카탈로그에서 ⚪ 미사용 후보였으나 2026-04-25 {{USER_NAME}} 지시로 활성 보조 위치 배정:
...
매트릭스 v1.1 통합 갱신은 다음 {{USER_NAME}} 픽 1라운드 끝나고 한꺼번에.
...
- {{USER_NAME}}의 "이번 받은 것 위주" 지시를 "활용도 올려라"로 해석. "기존 도구를 무시하라" 의미는 아님
