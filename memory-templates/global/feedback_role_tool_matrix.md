...
{{USER_NAME}} 요청 → 직군 매칭 → 이 맵에서 1순위 직진. 충돌 직군은 컨텍스트별 분기. 공백 직군은 보강 방안 따름. Founder 명시 지정 시 이 맵 무시.
...
> **v1.1 풀스택 가중치 (Profile v2.0)**: {{USER_NAME}}이 코드 직접 읽고 검수 가능. G-30~G-34 모두 영문 도구도 동등 평가, 한국어 보고는 효율 측면. typescript-reviewer/performance-optimizer/architect 활용 빈도 격상 가능. Codex 위임은 "능력 보완"이 아닌 "토큰·속도 효율" 위임.
...
- **기존 시스템 확장·문서화** (<owner workplace> SEED 등): `theming-system` + `pattern-library` + `documentation-template`
...
{{USER_NAME}}이 GAN 시리즈 활용 패턴 확인됨 (예: ux-writer 라운드 6). 풀스택 올라운더로서 자동 개선 루프 가치 큼.
...
{{USER_NAME}}이 작업 종류 키워드 던지면 L1이 자동으로 QA 직군 호출.
...
**핵심 발견**: {{USER_NAME}} "QA 항상 부족" 호소는 **도구 부족이 아니라 자동 트리거 부재** 문제로 추정. 트리거 룰을 박아 자동 호출이 핵심.
